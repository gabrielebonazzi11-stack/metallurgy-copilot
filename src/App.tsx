...
export default function App() {
  ...
  const callAI = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Errore dal backend.");
      }

      const aiText = data?.answer || data?.message || "Errore: il backend non ha restituito una risposta valida.";
      ...
      replaceMessagesInChat(chatId, [
        ...updatedMessages,
        {
          role: "AI",
          text:
            "⚠️ Errore backend. Controlla che il server sia acceso e che `/api/chat` funzioni.\n\n" +
